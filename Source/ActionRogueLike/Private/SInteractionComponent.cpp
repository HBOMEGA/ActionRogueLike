...
void USInteractionComponent::FindBestInteractable()
{
	...
	for (FHitResult Hit : Hits)
	{
		if ( bDebugDraw )
		{
			DrawDebugSphere(GetWorld(), Hit.ImpactPoint, TraceRadius, 32, LineColor, false, 5.0 );
		}
...
void USInteractionComponent::PrimaryInteract()
{
	if (!FocusedActor )
	{
		GEngine->AddOnScreenDebugMessage(-1, 1.0f, FColor::Red, "No FocusedActor to  Interact with!");
		return;
	}
